...
## 小技巧
   ...
   - 正则和非正则
      - `默认情况下，搜索和查询都是以正则方式进行匹配的，在模式串中遇到 . * [ / | < > ^ $ 时需要在前面加上“\”进行转义`
         - 例如查询 . 需要写成 \.
         - 例如查询 * 需要写成 \*
         - 例如查询 .* 需要写成 \.\*
      - :set magic   # 打开正则模式
   ...
   - 搜索和替换
      - 搜索
         - /搜索内容      # magic 形式的搜索
         - /\m搜索内容    # magic 形式的搜索
         - /\M搜索内容    # no magic 形式的搜索
         - /\v搜索内容    # very magic 形式的搜索
         - /\V搜索内容    # very no magic 形式的搜索
         - /\c搜索内容    # 大小写不敏感的 magic 形式的搜索
      - 替换
         - x 和 y 分别代表 x line 到 y line 的内容
         - 替换模式默认是正则模式（模式串关键字符需要转义），如果需要非正则替换方式需要以 :x,ysno方式
         - :x,ys/字符串1/字符串2/[g][c]     # x到y行内的内容把 字符串1 替换为 字符串2
            - g 和 c 是可选参数，如果加上 g 表示全局替换，如果加上 c 表示查找到一次在替换时进行会话式询问
            - x,ysno/字符串1/字符串2/[g][c] # 以非正则方式进行替换
         - :x,$/字符串1/字符串2/[g][c]     # 从 x 行到文章末尾进行替换
         - :x,yg/字符串/d                   # 将一行中有匹配到 字符串 的行进行删除
         - :x,yv/字符串/d                   # 将一行中没有匹配到 字符串 的行进行删除
   - 文件补全
      - ctrl + x 后
         - ctrl + p 向前切换成员
         - ctrl + n 向后切换成员
...
## 分屏
   - :vsp <A文本>                  # 当前文本和 A文本 进行垂直分屏，如果不写 A文件 则是当前文件与当前文件垂直分屏
   - :sp  <A文本>                  # 当前文本和 A文本 进行水平分屏，如果不写 A文件 则是当前文件与当前文件水平分屏
   - ctrl + w 方向                 # 分屏后，光标移动到相应方向的分屏内容中
   - ctrl + w =                    # 所有分屏内容占据划分比例相同
   - ctrl + w |                    # 如果是垂直分屏，则当前分屏占用全部屏幕
   - ctrl + w _                    # 如果是水平分屏，则当前分屏占用全部屏幕

## 内容搜索
   - 完全避免转义符的纯搜索方案
      - https://www.codenong.com/1550734/
   - 命令模式 /\c搜索内容           # 此方案可以不区分大小写进行搜索
   - :set noignorecase              # 设置大小写不敏感，搜索时可以不用输入 \c
...
## 正则表达式
   - 对于匹配的四种模式
      - https://zhuanlan.zhihu.com/p/56991272
   - 区别
      ...
      - 命令模式下大小写转换
         - 指令
            ```
            参考地址：https://blog.csdn.net/hunter___/article/details/82251557
            ...
            \l\组号   # 将分组第一个转换成小写
            \L\组号   # 将整个分组转换成小写
            转换是用分组进行执行的，上面的四个转义序列在其被显式停止前，对跟在其后的内容持续作用；
            如果要停止，需要使用'\e'进行中断
            源字符串为：every day
            :s/\(.*\) \(.*\)/\U\1\e \2/g          # 这会将 every转为大写后停止转换，结果为 EVERY day
            ```
         - 示例:
            ```
            :%s/\v(\w)(\w*)/\u\1\L\2/g

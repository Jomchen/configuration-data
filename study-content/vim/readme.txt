...
## 分屏
   ...
   - 方向
      - ctrl + w + 方向               # 分屏后，光标移动到相应方向的分屏内容中（方向可以用 h j k l 代替，也可以用方向键代替）
   - 调整分屏大小
      - ctrl + w =                    # 所有分屏内容占据划分比例相同
      - ctrl + w |                    # 如果是垂直分屏，则当前分屏占用全部屏幕
      - ctrl + w _                    # 如果是水平分屏，则当前分屏占用全部屏幕
      - 数字 + ctrl + w >             # 水平分屏下调节高度（数字决定调整多大）
         - > 也可替换为 <
      - 数字 + ctrl + w +             # 垂直分屏下调节宽度（数字决定调整多大）
         - + 也可替换为 -
      - res 数字                      # 水平分屏下设置高度
      - vertical res 数字             # 垂直分屏下设置宽度
...
## 内容搜索
   - 命令行搜索
      ...
      - 命令行搜索 当执行了搜索后，n: 正向矢量搜索；N: 反向矢量搜索
   - 行搜索
      - 搜索
         - f 字母   # 行内光标定位到右边匹配的字母处
         - F 字母   # 行内光标定位到左边匹配的字母处
      - 正/反向搜索
         - ;        # 正向矢量搜索
         - ,        # 反向矢量搜索
   - 行匹配操作
      - :%g/正则串/d    # 全文匹配正则串的行进行删除（如果是复制则将最后的 d 改为 y）
      - :%v/正则串/d    # 全文不匹配正则串的行进行删除（如果是复制则将最后的 d 改为 y）
...
## 正则表达式
   ...
   - 区别
      - 在 vim 中使用的正则表达式差不多只有 . * [] 元字符可以直接用,其它元字符只能加 \
         ...
         加上行号 :%s/^/\=line(".")/g
         加上行号和一个空格 :%s/^/\=line(".")." "/g
         对行号进行计算（这里以行号加1作为结果） :%s/^/\=line(".") + 1/g

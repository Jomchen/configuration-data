...
public class TreeTool<Obj, Identifier> {
    ...
    public List<Obj> getTree(int depth, List<Obj> originalRootList) {
        if (Objects.isNull(acquireIdentifierOfItSelfFunction) || Objects.isNull(acquireChildrenByParentIdentifierFunction) || Objects.isNull(setChildrenFunction)) {
            throw new HumiRuntimeException(ResponseCode.FAILED, "Building conditions is not complete.");
        }
        if (depth <= 0) { throw new HumiRuntimeException(ResponseCode.FAILED, "The depth of tree is at least 1"); }
        if (CollectionUtils.isEmpty(originalRootList)) { throw new HumiRuntimeException(ResponseCode.FAILED, "Root element must be not empty."); }
